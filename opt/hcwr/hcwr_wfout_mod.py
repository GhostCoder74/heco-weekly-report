...
def generate_report(work_hours, kw_should, contract_hours, feiertage, urlaub, abwesend,
    ...
    report_lines.append(
        f"Arbeitsstunden: {format_decimal(work_hours/3600)} von {format_decimal(kw_should/3600)} "
        f"(Vertrag: {format_decimal(contract_hours)}  Feiertage: {format_decimal(feiertage/3600)}  "
        f"Urlaub: {format_decimal(urlaub/3600)}  abwesend: {format_decimal(abwesend/3600)})"
    )
    if int(HCWR_GLOBALS.DBG_LEVEL) == -100:
        debug(f"sign = {HCWR_GLOBALS.SIGN}")
        debug(f"kw_overhours_add = {format_decimal(kw_overhours_add)}")
    kwOLD = format_decimal(kw_old_overhours/3600)
    kwADD = float(f"{(kw_overhours_add/3600)}")
    ZK = format_decimal(kw_stundenkonto/3600)
    if kwADD < 0:
        sign = "-"
    else:
        sign = "+"
    report_lines.append(
        f"Stundenkonto: {kwOLD} {sign} {format_decimal(kwADD)} "
        f"= {ZK}"
    )
